...
pool.on("error", (err, _client) => {
  logger.error("[postgres pool] idle client error:", { message: err?.message || err, code: err?.code || "" });
});
...
pool
  ...
  .then((res) => logger.info("[postgres] connected:", res.rows[0]))
  .catch((err) =>
    logger.error(
      `[postgres] initial connect failed (${err?.code || err?.message}). Unread in-app notifications will return empty until POSTGRES_HOST is reachable.`
    )
  );
